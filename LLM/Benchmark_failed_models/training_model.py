...
#MODEL_NAME='' # Model from Hugging Face
#MODEL_RES='ClinicalT5-base_model'
OPTIMIZER_RES='optimizer'


### Functions
def save_results_to_csv(results, lr_train_history, method_name):
    ...
    try:
        ...
        filepath_lr = os.path.join(SAVE_FOLDER, f"lr_history_{method_name}_{timestamp}.csv")
        ...
        print(f"Saving lr history to: {filepath_lr}")

        df_lr = pd.DataFrame(lr_train_history)
        df_lr.to_csv(filepath_lr, index=False)

        if os.path.exists(filepath_lr):
            print(f"File created successfully: {filepath_lr}")
        else:
            print("File was not created!")
            return None
...
def training_test_validation(num_epochs, device, num_labels=2):
    ...
    print(f"Modello scelto: {MODEL_NAME}")
    data_loader = PIIDataLoader(train_texts, train_labels, valid_texts, valid_labels, test_texts, test_labels, tokenizer_name=MODEL_NAME)
    ...
    for lr in [ 1e-5, 2e-5, 3e-5, 4e-5, 5e-5]:
        ...
        perEpoch_test_results, lr_train_history = model_trainer.train(train_loader, valid_loader, test_loader)
        ...
        if perEpoch_test_results is not None:
            perEpoch_test_results.to_csv()
        # results = [{
        ...
        # Save results to CSV
        save_results_to_csv(perEpoch_test_results, lr_train_history, 'standard')
    return perEpoch_test_results
...
def kfold_training(n_splits, num_epochs, device, num_labels=2):
    ...
    for fold, (train_texts, train_labels, test_texts, test_labels) in enumerate(data_handler.kfold_split_data()):
        ...
        data_loader = PIIDataLoader(train_texts, train_labels, [], [], test_texts, test_labels, tokenizer_name=MODEL_NAME)
...
def arg_commandline():
    ...
    parser.add_argument('--dataset', type=str, default='../../Dataset/combined_shuffled_final.csv', help='Path to the dataset file')
    parser.add_argument('--num_labels', type=int, default=2, help='Number of classification labels')  # Nuovo parametro
    parser.add_argument('--model', type=str, default='hossboll/clinical-t5', help='address of the model')
...
def main():
    ...
    global MODEL_RES
    MODEL_RES = MODEL_NAME + "_res_model"
    global SAVE_FOLDER
    SAVE_FOLDER = MODEL_NAME + '_results_train'  # Path where to save the data
